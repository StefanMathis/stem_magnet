...
the documentation of the [`ArcSegmentMagnet`] struct.
 */
fn main() {
    air_gap_radius();
    center_thickness();
...
fn air_gap_radius() {
    ...
    texts.push(Text::new(
        "Shared parameters: core_radius = 5 m, side_thickness = 1 m, angle = PI/6".into(),
        Anchor::Center,
        [0.0, 0.0],
        [1.5 * dx, -2.0],
    ...
    texts.push(Text::new(
        "Shared parameters: core_radius = -6 m, side_thickness = 1 m, angle = PI/6".into(),
        Anchor::Center,
        [0.0, 0.0],
        [1.5 * dx, 1.5],
        Color::new(0.0, 0.0, 0.0, 1.0),
    ...
    bb.scale(1.001);
    let bb = BoundingBox::new(bb.xmin(), bb.xmax(), bb.ymin() - 1.0, bb.ymax());
    let view = Viewport::from_bounding_box(&bb, SideLength::Long(800));

    let fp = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join(&format!("docs/img/arc_segment_vary_air_gap_radius.svg"));
...
fn center_thickness() {
    ...
    let fp = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join(&format!("docs/img/arc_segment_vary_center_thickness.svg"));
    view.write_to_file(&fp, |cr| {
        cr.set_source_rgb(1.0, 1.0, 1.0);
        cr.paint()?;
